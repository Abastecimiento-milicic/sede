...
(function () {
  ...
  window.fetchWithCache = async function fetchWithCache(url) {
    ...
    try {
        ...
        throw new Error(`Error de red al intentar descargar: ${fetchError.message}`);
    }

    if (resp.type === 'opaqueredirect') {
        console.warn("[cache-utils] Redirección opaca detectada (posible Azure AD Login). Recargando...");
        window.location.reload(true);
        // Devolvemos una promesa que nunca se resuelve para evitar que el script continúe fallando
        return new Promise(() => {});
    }

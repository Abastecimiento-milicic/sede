(function() {
  const csvUrl = "./REPORTE CUMPLIMIENTO SS Y SUB.csv";
  const DELIM = ";";
  ...
  async function fetchWithCache(url) {
      const version = (window.VERSION_ACTUALIZACION || "v1").replace(/[^a-zA-Z0-9_-]/g, "");
      const CACHE_NAME = `servicios-data-cache-${version}`;
      try {
          const cache = await caches.open(CACHE_NAME);
          const cachedResponse = await cache.match(url);
          if (cachedResponse) {
              console.log("Cargando servicios desde cache...");
          ...
          console.error("Cache error, fetching directly:", e);
          const response = await fetch(url);
          return response.text();
      }
  }

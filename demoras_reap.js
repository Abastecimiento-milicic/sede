...
/* ============================
   CONFIG
   ============================ */
const csvUrl = "./R1 - REPORTE CUMPLIMIENTO REAP ACUMULADO.csv";
const DELIM = ";";
...
function getFilteredRows() {
  ...
  return data.filter(r => {
    ...
    const matchCentro = !centros.length || centros.includes(clean(r[CENTRO_COL]));
    ...
    return matchClient && matchCentro && matchGc && matchGcCompra && matchComprador && matchMes;
  });
}
...
function getTrendRows() {
  ...
  return data.filter(r => {
    ...
    const matchGcCompra = !gcs_compra.length || gcs_compra.includes(clean(r[GRUPO_COMPRA_COL]));
    const matchComprador = !comps.length || comps.includes(clean(r[COMPRADOR_COL]));
    return matchClient && matchCentro && matchGcCompra && matchComprador;
  });
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  loadData()
    .then(text => {
      ...
      // Event listeners
      ["clienteSelect", "centroSelect", "caracterSelect", "grupoCompraSelect", "compradorSelect", "mesSelect"].forEach(id => {
